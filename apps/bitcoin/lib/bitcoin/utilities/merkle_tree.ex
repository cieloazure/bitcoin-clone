...
defmodule Bitcoin.Utilities.MerkleTree do
  ...
  def calculate_hash(items) when is_list(items) do
    items = if rem(length(items), 2) != 0 do
    ...
    merkle_tree = Enum.reduce(0..trunc(:math.log2(length(items))), %{}, fn x, acc -> Map.put(acc, x, []) end)
  ...
  def authentication_path(merkle_tree, _leaf \\ nil) do
    ...
    auth = Enum.reduce(0..max_height - 1, %{}, fn x, acc -> 
      level = Map.get(merkle_tree, x)
      item = Enum.at(level, 1)
      Map.put(acc, x, item) 
    end)

    stack = Enum.reduce(0..max_height - 1, %{}, fn x, acc -> 
      level = Map.get(merkle_tree, x)
  ...
  defp calc_auth_path(auth, stack, leaf, auth_paths, max_height, merkle_tree, nodes) when leaf < nodes do
  ...
  defp update_stack(stack_collection, merkle_tree, height, max_height, max_nodes) when height < max_height do
    ...
    {_, stack_collection} = Map.get_and_update(stack_collection, height, fn current_value -> 
  ...
  defp update_stack_proc(stack, _, height, start_node) when is_nil(height) and is_nil(start_node) and length(stack) == 1 do
    stack
  end
  ...
  defp update_auth(leaf, height, factor, auth, stack, max_height) when rem(leaf + 1, factor) == 0 and height < max_height do
    {stack1, _, _} = Map.get(stack, height)
    stack_val = if Stack.empty?(stack1) do
      nil
    else
      List.first(stack1)
    end
    ...
    {_, stack} = Map.get_and_update(stack, height, fn current_value -> 
  ...
  defp tree_hash(items, stack, output) do
    ...
    if !is_nil(peek) and equal_height?(peek) do
      ...
      {_, output} = Map.get_and_update(output, height + 1, fn current_array -> 
      ...
      {_, output} = Map.get_and_update(output, 0, fn current_array -> 
